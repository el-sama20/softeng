...
const app = {
    ...
    updatePayrollExportLabel: () => {
        ...
        if (historyLabelEl) {
            ...
            if (selectedPeriod === 'all') {
                historyLabelEl.innerText = historyMode === 'monthly' ? 'Export All Months History' : 'Export All Weeks History';
        ...
        return Array.from(groups.values())
            .map(group => {
                ...
                return {
                    ...
                    reimbursedBy: status,
                    statusLabel: group.reimbursedCount === group.periodCount
                        ? 'Reimbursed'
                        : (group.reimbursedCount > 0 ? 'Partly reimbursed' : 'Not reimbursed yet'),
                    statusClass,
                    earliestStart: group.earliestStart,
    ...
    getSelectedPayrollHistoryExportSummary: () => {
        ...
        const periodLabel = isSinglePeriod
            ? (mode === 'monthly' ? `${firstPeriod.label} (${rangeLabel})` : firstPeriod.label)
            : (mode === 'monthly' ? `${history.length} payroll months (${rangeLabel})` : `${history.length} payroll weeks`);
        const salaryDay = mode === 'weekly' && isSinglePeriod ? firstPeriod.salaryDate : null;
        const periodStatus = isSinglePeriod
            ? (mode === 'monthly'
                ? (firstPeriod.statusLabel || firstPeriod.reimbursedBy)
            ...
            if (historyMode === 'monthly') {
                ...
                tr.innerHTML = `
                    <td>
                        <div class="history-period-cell">
                            ...
                            <small>${period.periodCount} completed week${period.periodCount === 1 ? '' : 's'}${period.latestReimbursedAt ? ` - Last reimbursed by ${escapeHTML(period.reimbursedBy)}` : ''}</small>
                        </div>
                    </td>
                    <td>${escapeHTML(period.coveredRange)}</td>
                    <td><span class="history-status ${period.statusClass || 'history-status-warning'}">${escapeHTML(period.statusLabel || period.reimbursedBy)}</span></td>
                ...
                tr.innerHTML = `
                    <td>
                        <div class="history-period-cell">
                            <strong>${escapeHTML(period.label)}</strong>
                            <small>${period.reimbursedAt ? `Reimbursed ${escapeHTML(formatDateTime(period.reimbursedAt))}${period.reimbursedBy ? ` by ${escapeHTML(period.reimbursedBy)}` : ''}` : 'Not marked reimbursed yet'}</small>
                        </div>
                    </td>
                    <td>${escapeHTML(formatDisplayDate(period.salaryDate || period.endDate))}</td>
                    <td><span class="history-status ${period.status === 'reimbursed' ? 'history-status-paid' : 'history-status-warning'}">${escapeHTML(period.status === 'reimbursed' ? 'Reimbursed' : 'Not reimbursed yet')}</span></td>
                    <td>${escapeHTML(formatCurrency(period.totalAmount))}</td>
                    <td>
                        ${canMarkWeekly
